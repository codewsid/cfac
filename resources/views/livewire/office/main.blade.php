<div class="max-w-[95%] mx-auto sm:px-6 lg:px-0 mt-3">
    ...
    {{-- @if (\Illuminate\Support\Facades\Hash::check('password', auth()->user()->password))
        <div class="fixed inset-0 bg-black/50 flex justify-center items-start">
            <div class="bg-white mt-20 p-3 w-2/6 rounded-md" x-data="{ banner: true }">
                <h1 class="font-semibold mb-2 text-gray-500" x-show="!banner" x-cloak>Change default password</h1>
                <section x-show="banner" class="bg-kgreen/30 rounded p-3 relative" x-cloak>
                    <button @click="banner = false" class="absolute right-2 top-2">
                    ...
                    <h1 class="font-semibold">It seems that this is your first time to login here.</h1>
                    <p class="text-sm">You are required to change your default password.</p>
                </section>

                <div class="mt-3 space-y-3">
                    <div>
                        <x-group-input type="password" model="" label="Password" placeholder="Enter password"
                            name="password" />
                        <span class="text-sm font-semibold text-red-500 italic">Error</span>
                    </div>
                    <div>
                        <x-group-input type="password" model="" label="Confirm Password"
                            placeholder="Confirm password" name="confirmation" />
                        <span class="text-sm font-semibold text-red-500 italic">Error</span>
                    </div>

                    <div class="flex justify-end">
                        <button wire:click="updatePassword" class="bg-kgreen text-white rounded-md py-2 px-5">Save
                            changes</button>
                    </div>
                </div>
            </div>
